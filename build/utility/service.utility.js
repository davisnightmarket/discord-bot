...
async function GetGuildServices(guildId) {
    if (!servicesByGuildId.has(guildId)) {
        ...
        const orgCoreService = new service_1.NmOrgService(config.GSPREAD_ORG_ID);
        const personCoreService = new service_1.NmPersonDataService(config.GSPREAD_PERSON_ID);
        servicesByGuildId.set(guildId, {
            config,
            nightDataService: new service_1.NmNightDataService(config.GSPREAD_NIGHT_ID, personCoreService),
