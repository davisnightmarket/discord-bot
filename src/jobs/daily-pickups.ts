import { roleMention, type Guild, bold, userMention, EmbedBuilder, hyperlink } from "discord.js";
...
export async function DailyPickupsUsingEmbed(guild: Guild, services: GuildServiceModel) {
    const channel = getChannelByName(guild, today())

    const pickups = await services.pickupsDataService.getPickupsFor(today())
    const pickupDescriptions = await Promise.all(pickups
        .map(async (pickup) => `* [${pickup.org}](https://www.example.com) at ${pickup.time}: ${await getVolunteerList(services, pickup)}`))
...
export async function DailyPickupsWithoutThread(guild: Guild, services: GuildServiceModel) {
    const channel = getChannelByName(guild, today())
    ...
    for (const pickup of pickups) {
        channel.send([
            `${bold(pickup.org)} at ${pickup.time}: ${await getVolunteerList(services, pickup)}`,
        ].join("\n"))
    }
}
...
async function getVolunteerList(services: GuildServiceModel, pickup: PickUp) {
    const people = await Promise.all(
        [pickup.volunteer1, pickup.volunteer2, pickup.volunteer3]
            ...
            .map(async (name) => await services.personCoreService.getPerson({ name }) ?? { name })
    )
    ...
    return people
        ...
        .map(person => person.discordId ? userMention(person.discordId) : person.name)
        .join(", ")
}
